...
# 
#   DESCRIPTION: Update and then Backup OES configuration files using 
#                oes_upgrade_check.pl script. Do a backup of the server 
#                and finally take a supportconfig.
#
...
#  LAST UPDATED: Fri Nov 23 2018 10:13
#      REVISION: 2.4
#     SCRIPT ID: ---
# SSC SCRIPT ID: --
#===============================================================================
...
if [ ${admfdn} = ${cadm} ]; then
  echo -e "The account listed is: $cadm"                                      | tee -a $rep
  ...
  echo -e "The oes-ldap config file has been updated with your admin account($admfdn)." | tee -a $rep
fi
sleep 5
clear
...
tar jcvf ${bkdir}/oes2018_prep-${host}.tbz ${bkdir}/backup_etc*.tbz ${bkdir}/backup_home*.tbz  ${bkdir}/backup_root*.tbz  ${bkdir}/backup_opt*.tbz ${bkdir}/backup_var*.tbz  ${bkdir}/oes2015_*.tbz ${bkdir}/nts_*.tbz ${bkdir}/nts_*.tbz.md5 >> $log 2>> $errlog
echo "---------------------------------------------------------" | tee -a $rep
