...
#  LAST UPDATED: Wed Nov 21 2018 14:26
#      REVISION: 2.3
#     SCRIPT ID: ---
# SSC SCRIPT ID: --
#===============================================================================
version=0.2.3                                   # version number of the script
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
  echo -e "\nContinuing with Upgrade check of $host"
else
  echo -e "\nPlease run this script again when you are ready"    | tee -a $rep
  echo -e "to prepare for an upgrade." | tee -a $rep
  exit 1
fi
...
echo "---------------------------------------------------------" | tee -a $rep
echo -e "There are $oes11sp1 configuration files in /etc/sysconfig/novell"  | tee -a $rep
echo ""                                                          | tee -a $rep
echo -e "Checking for old OES11 SP2 config files"                | tee -a $rep
echo "---------------------------------------------------------" | tee -a $rep
echo -e "There are $oes11sp2 configuration files in /etc/sysconfig/novell"  | tee -a $rep
echo ""                                                          | tee -a $rep
echo -e "Checking for OES2015 SP1 config files"                  | tee -a $rep
echo "---------------------------------------------------------" | tee -a $rep
echo -e "There are $oes2015sp1 configuration files in /etc/sysconfig/novell"| tee -a $rep
echo ""                                                          | tee -a $rep
echo -e "Backing up any old config files under /etc/sysconfig/novell"       | tee -a $rep
echo "---------------------------------------------------------" | tee -a $rep
...
echo ""                                                          | tee -a $rep
echo -e "Backing up OES2015 SP1 config files"                    | tee -a $rep
echo "---------------------------------------------------------" | tee -a $rep
...
if [ "$tree" == CLUSTER-TREE ]; then
  ...
  if [ $(cat /tmp/urip.tmp.$$ | cut -f 4 -d '.') == 193 ]; then
    ...
    echo -e "The eDir replica IP address in the edir_oes2015_sp1 file is correct for $tree" | tee -a $rep
  else
    echo -e "There is either no IP Address, or the wrong IP Address in edir_oes2015_sp1, going to correct this." | tee -a $rep
...
if [ $pch -ge 1 ]; then
  ...
  echo -e "A reboot may be required for the recent patching"     | tee -a $rep
fi
sleep 5
clear
...
read -p "Do you want to continue with a configuration check? (Y|N): " -n 1 -r
  if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    echo -e "\nUpgrade check skipped."                           | tee -a $rep
  fi
sleep 5
clear
...
echo "Backup of /var is complete."                               | tee -a $rep
    ...
    echo -e "\nBackup of files and folders skipped."             | tee -a $rep
  fi
sleep 5
clear
...
echo -e "Supportconfig complete."                                | tee -a $rep
sleep 5
clear
...
echo "Creating a single archive file under ${bkdir}"             | tee -a $rep
...
/bin/rm ${bkdir}/backup_etc*.tbz
/bin/rm ${bkdir}/backup_home*.tbz
/bin/rm ${bkdir}/backup_root*.tbz
/bin/rm ${bkdir}/backup_opt*.tbz
/bin/rm ${bkdir}/backup_var*.tbz
/bin/rm ${bkdir}/oes2015_sp1_*.tbz
/bin/rm ${bkdir}/nts_*.tbz
/bin/rm ${bkdir}/nts_*.tbz.md5
