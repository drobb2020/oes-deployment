...
##############################################################################
#
#    lab-setup.sh - Bash script to import ldif information into a new tree
...
# Last updated: Thu Dec 28 11:16 2017
...
PWD=yellowStone!228
SRCFLDR=/root/setup
LOG=/root/reports/ice-logs/excs-setup.log
...
function helpme() { 
	...
	echo "for example ./excs-setup.sh lab"
	echo "========================================"
}
...
function reminder() { 
	echo "--[ Reminder ]----------------------------------------------------------------"
	echo "Please remember that you might need to modify the excs-tree-compelte.ldif file"
        echo "for the tree you are setting up. If you have already done so answer yes to"
...
if [ $# -lt 1 ]
    then
	$NDSBIN/ice -l $LOG -S LDIF -f $SRCFLDR/lab-tree.ldif -v -D LDAP -s $HOST -p $PORT -L -d $ADM -w $PWD -v
